  ...
  const chartData = useMemo(() => {
    ...
    if (baseAverageDailySales > 0 && (availableStock > 0 || sortedReorderSchedule.length > 0)) {
      ...
      while (dayOffset <= maxDays && !hasReachedZero) {
         ...
         // 해당 날짜에 입고가 있는지 확인 (입고는 판매 전에 먼저 반영)
         // 모든 입고 일정을 순차적으로 확인하여 해당 날짜 또는 이전 날짜의 입고를 반영
         while (reorderIndex < sortedReorderSchedule.length) {
           ...
           if (reorder.date <= nextDateStr) {
             // 해당 날짜 또는 이전 날짜의 입고 반영 - 입고 후 재고가 증가하면 판매 추세도 증가
         ...
         if (currentForecastStock > 0) {
           ...
           dailySales = baseAverageDailySales * currentStockRatio;
         ...
         const stockValue = Math.max(0, Math.round(currentForecastStock));
...
console.log('SalesChart.tsx has been recreated successfully');
